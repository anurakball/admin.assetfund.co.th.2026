...
| ข้อมูลหน้าแรก > ลิงก์ด่วน | `HomeSamText` | `web_core_single` / 1 (แถวเดียว) | ไทล์ 3 อัน (ผลการดำเนินงาน / NAV ทั้งหมด / ปฏิทินกองทุน) · รูป `Files/Site0/1/home/tiles/*.jpg` | **front-end ต่อแล้ว + Preview ใช้ได้** (18 ก.ย. 2569) — `Services/SqlHomeQuickTileService.cs` → partial `_NavPrices` / V2 / V3 — ดูสเปกด้านล่าง |
...
**`HomeSamText` — ข้อมูลหน้าแรก > ลิงก์ด่วน (18 ก.ย. 2569) — สิ่งที่ front-end ทำตามอยู่** (`SqlHomeQuickTileService.cs` + partial `_NavPrices` / `V2` / `V3` — ยึดแนว "SAM ใส่ใจ": `web_core_single` ระเบียนเดียว, แก้ไข + อนุมัติ, front-end อ่าน `pb_*` + HomeGate, โหลดเฉพาะเมื่อ layout มี widget ที่ใช้):
- เปิดคืนเมนู SAM `HomeSamText` (module 1) ในกลุ่ม "ข้อมูลหน้าแรก" ถัดจาก "รูปสไลด์หน้าแรก" ชื่อ **"ลิงก์ด่วน"** ไอคอน `fa-solid fa-signs-post` · `CanAdd/Delete/Status/Move = false` · `ListData_Default` · ฟอร์มใหม่ทั้งไฟล์ `Views/HomeSamText/Edit.cshtml` (4 fieldset: ข้อมูลหลัก + ไทล์ 1–3, ไม่มี cms-note)
