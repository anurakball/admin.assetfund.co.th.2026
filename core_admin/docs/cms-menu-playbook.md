...
| ข้อมูลหน้าแรก > มูลค่าหน่วยฯ (3 Links) | `HomeSamText` | `web_core_single` / 1 (แถวเดียว) | ไทล์ 3 อัน (ผลการดำเนินงาน / NAV ทั้งหมด / ปฏิทินกองทุน) · รูป `Files/Site0/1/home/tiles/*.jpg` | **front-end ต่อแล้ว + Preview ใช้ได้** (18 ก.ย. 2569) — `Services/SqlHomeQuickTileService.cs` → partial `_NavPrices` / V2 / V3 — ดูสเปกด้านล่าง |
...
**`HomeSamText` — ข้อมูลหน้าแรก > มูลค่าหน่วยฯ (3 Links) (18 ก.ย. 2569) — สิ่งที่ front-end ทำตามอยู่** (`SqlHomeQuickTileService.cs` + partial `_NavPrices` / `V2` / `V3` — ยึดแนว "SAM ใส่ใจ": `web_core_single` ระเบียนเดียว, แก้ไข + อนุมัติ, front-end อ่าน `pb_*` + HomeGate, โหลดเฉพาะเมื่อ layout มี widget ที่ใช้):
- เปิดคืนเมนู SAM `HomeSamText` (module 1) ในกลุ่ม "ข้อมูลหน้าแรก" ถัดจาก "รูปสไลด์หน้าแรก" ชื่อ **"มูลค่าหน่วยฯ (3 Links)"** ไอคอน `fa-solid fa-signs-post` · `CanAdd/Delete/Status/Move = false` · `ListData_Default` · ฟอร์มใหม่ทั้งไฟล์ `Views/HomeSamText/Edit.cshtml` (4 fieldset: ข้อมูลหลัก + ไทล์ 1–3, ไม่มี cms-note)
- ฟิลด์ (`Field_HomeQuickTiles` ใน `AdminMenu.cs` — ไม่ใช้ `FieldUpdate_CoreSingle` ทั้งชุด): `title` = ชื่อรายการ (หลังบ้านเท่านั้น, บังคับ) · ไทล์ที่ n, b = (n−1)×6: `t{b+1}`/`en_` หัวข้อ (**ไทยว่าง = ไทล์ไม่แสดงทุกภาษา**) · `t{b+2}`/`en_` คำอธิบาย · `t{b+3}`/`en_` URL (ต้องขึ้นต้น `/` หรือ `http(s)://` — ว่าง = ไม่ลิงก์ เรนเดอร์เป็น div) · `t{b+4}` Target (`_blank` = `target=_blank rel=noopener`) · `t{b+5}` รูป (v1 พื้นหลัง / v2 รูปย่อ — ว่าง = ไม่มี img; v2 ซ่อนรูปย่อบนมือถือเป็นดีไซน์เดิม) · `t{b+6}` ไอคอน Bootstrap Icons `^bi-[a-z0-9-]+$` (v3 — ว่าง/ผิดรูปแบบ = ไอคอนเดิมของตำแหน่งนั้น) · อังกฤษว่าง = ไทย
- front-end: model `NavPricesSection { Nav, Tiles }` แทน `FundNavSummary` ของ partial ทั้ง 3 · `HomeController.BuildHomeViewModel` ดึง NAV + ไทล์ **ครั้งเดียวต่อ request เฉพาะเมื่อ layout มี NavPrices*** (ยืนยันด้วย Extended Events: 1/2/3 widget = NAV 1 + ไทล์ 1 query, ไม่มี widget = 0) · NAV ว่าง = ทั้ง section ไม่แสดง · ไม่มีไทล์ = ตาราง NAV `col-12` เต็มแถว · โทนของ v1 (feature/accent/muted) ตามลำดับที่แสดง
- gate: `status = 1 AND show_front = 1 AND (pb_issue_date_config = '1' OR ช่วงวันที่)` (`issue_date_config` เป็น nvarchar(1)) · พรีวิว `/_preview/page/HomeSamText/1?lang=` (PreviewMap `web_core_single`, 1, BoxDataHome) = หน้าแรกที่ไทล์เป็นฉบับร่าง
- page builder: widget NavPrices / V2 / V3 มี `mod_name = HomeSamText` (ปุ่มดินสอเปิด Edit/1) และ HTML ตัวอย่างใช้ token `|||pb_tN|||` แทนข้อความ/ลิงก์/รูป/ไอคอนของไทล์ (ตาราง NAV ในการ์ดยังเป็น snapshot)
- deploy: `docs/sql/2026-09-18-home-quick-tiles.sql` (idempotent — ขั้น 1 เขียนทับเนื้อหาแถว ข้ามได้ถ้าเซิร์ฟเวอร์แก้แล้ว) + อัปโหลด `wwwroot/Files/Site0/1/home/tiles/*.jpg`
