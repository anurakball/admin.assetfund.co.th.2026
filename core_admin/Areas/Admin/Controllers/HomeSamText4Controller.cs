...
{
    public class HomeSamText4Controller : AdminCoreController
    {
        public HomeSamText4Controller(IWebHostEnvironment hostingEnvironment, IConfiguration iConfig, IHttpContextAccessor iContext) : base(hostingEnvironment, iConfig, iContext)
        {
            Module = _admin.GetModule("HomeSamText4");
        }
        ...
        public override IActionResult Edit(int id)
        {
            ...
            try
            {
                ...
                var provinceDT = new System.Data.DataTable();
                provinceDT.Columns.Add("id");
                provinceDT.Columns.Add("name");
                ...
                try
                {
                    ...
                    cmd.CommandText = "SELECT province_id, thai_name FROM provinces ORDER BY thai_name ASC";
                    using var reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
                        provinceDT.Rows.Add(reader["province_id"].ToString(), reader["thai_name"].ToString());
                    }
                }
                catch { }
